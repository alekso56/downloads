...
public class PeripheralSilkTouchHosted extends TurtleBase
  implements IHostedPeripheral
{
  ...
  PeripheralSilkTouchHosted(ITurtleAccess turtle, TurtleSide side)
  {
  ...
  {
    return new String[] { "digSilkTouch", "digSilkTouchUp", "digSilkTouchDown" };
  }
    ...
    throws Exception
  {
    ...
    if (getOtherUpgradeName(this.turtleHosting, this.sideHosting) == "tofu")
    {
      ...
      ItemStack droppedItem = new ItemStack(blockID, 1, metadata);
      this.world.setBlockToAir(newX, newY, newZ);
      this.world.playAuxSFX(2001, newX, newY, newZ, blockID + metadata * 4096);
      store(this.turtleHosting, dir, droppedItem);

      return new Object[] { Boolean.valueOf(flag) };
    }
    if (getOtherUpgradeName(this.turtleHosting, this.sideHosting) == "Shearing")
    {
      if ((block.canSilkHarvest(this.world, this.playerTurtle, newX, newY, newZ, metadata)) && 
        (canTurtleHarvest(this.turtleHosting, item, block, metadata))) {
  ...
  {
    String[] allowed = { "Digging", "Mining", "Felling", "Farming", "Shearing", "Tofu" };
